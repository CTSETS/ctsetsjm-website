    ...
    <>
      ...
      <nav style={{ background: S.navy, borderBottom: "3px solid " + S.gold, position: "sticky", top: 0, zIndex: 1000, boxShadow: "0 2px 12px rgba(0,0,0,0.18)" }} role="navigation" aria-label="Main">
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", height: 72, justifyContent: "space-between" }}>
          ...
          <div className="desktop-nav" style={{ display: "flex", gap: 2, alignItems: "center" }} role="menubar">{NAV_GROUPS.map(g => <NavDropdown key={g.label} group={g} page={page} setPage={setPage} />)}<button onClick={() => setPage("Student Portal")} style={{ padding: "8px 14px", borderRadius: 6, background: S.emerald, color: "#fff", fontSize: 11, fontWeight: 700, fontFamily: S.body, border: "none", cursor: "pointer", whiteSpace: "nowrap", marginLeft: 4 }}>🎓 Student Portal</button></div>
