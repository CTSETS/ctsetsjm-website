      ...
      {hovered && (
        <div style={{
          ...
          {group.children.map(c => {
            ...
            // Internal link — uses setPage
            return (
              <button
                key={c.page}
                onClick={() => { setPage(c.page); setHovered(false); }}
                style={{
                  ...
                  fontSize: 13, fontWeight: page === c.page ? 700 : 400,
                  cursor: "pointer", fontFamily: S.body,
      ...
      >
          ...
          <div className="desktop-nav" style={{ display: "flex", gap: 2, alignItems: "center" }} role="menubar">
            ...
            <a
              ...
              style={{
                ...
                fontFamily: S.body, border: "none", cursor: "pointer",
                whiteSpace: "nowrap", marginLeft: 4,
                boxShadow: "0 2px 10px rgba(37,99,235,0.3)",
                transition: "all 0.2s", textDecoration: "none",
                display: "inline-flex", alignItems: "center", gap: 4,
              }}
            >🎓 Student Portal <span style={{ fontSize: 9, opacity: 0.7 }}>↗</span></a>
          </div>
        ...
        {open && (
          <div style={{ background: "#0B1120", borderTop: "1px solid rgba(37,99,235,0.08)", padding: "8px 0 16px" }} role="menu">
            {NAV_GROUPS.map(g => {
              if (!g.children) {
                return (
                  <button
                    key={g.label}
                    onClick={() => { setPage(g.page); setOpen(false); }}
                    style={{
                      ...
                      fontSize: 14, fontWeight: page === g.page ? 700 : 400,
                      cursor: "pointer", fontFamily: S.body, textAlign: "left",
                    }}
                  >{g.label}</button>
                );
              }
              ...
              return (
                <div key={g.label}>
                  <div style={{
                    padding: "10px 24px 4px", fontSize: 10, color: S.gold,
                    fontFamily: S.body, letterSpacing: 2, textTransform: "uppercase", fontWeight: 700
                  }}>{g.label}</div>
                  ...
                  {g.children.map(c => {
                    ...
                    // Internal link in mobile menu
                    return (
                      <button
                        key={c.page}
                        onClick={() => { setPage(c.page); setOpen(false); }}
                        style={{
                          display: "block", width: "100%", padding: "10px 40px",
                          border: "none",
                          background: page === c.page ? "rgba(37,99,235,0.1)" : "transparent",
            ...
            {/* Student Portal — external in mobile */}
            <a
              href={PORTAL_URL}
              target="_blank"
              rel="noopener noreferrer"
            ...
            >🎓 Student Portal <span style={{ fontSize: 9, opacity: 0.5 }}>↗</span></a>
          </div>
        )}
      </nav>
    </>
