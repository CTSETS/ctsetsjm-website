...
export default function Footer({ setPage }) {
  var link = function(label, page) {
    return <button key={label} onClick={function() { setPage(page || label); }} style={{ display: "block", background: "none", border: "none", color: "rgba(255,255,255,0.6)", fontSize: 12, fontFamily: S.body, cursor: "pointer", padding: "3px 0", textAlign: "left" }}>{label}</button>;
  };

  return (
    <footer style={{ background: S.navy, padding: "40px 20px 20px", borderTop: "3px solid " + S.gold }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr", gap: 28, marginBottom: 28 }} className="resp-grid-2">
          ...
          {/* Column 2: Admissions */}
          <div>
            <div style={{ fontSize: 10, color: S.gold, letterSpacing: 2, textTransform: "uppercase", fontFamily: S.body, fontWeight: 700, marginBottom: 12 }}>Admissions</div>
          ...
          <div>
            ...
            <button onClick={function() { setPage("Student Portal"); }} style={{ display: "block", background: "none", border: "none", color: S.emerald, fontSize: 12, fontFamily: S.body, padding: "3px 0", cursor: "pointer", fontWeight: 600, textAlign: "left" }}>Student Portal →</button>
          </div>

          {/* Column 4: Support */}
          <div>
            <div style={{ fontSize: 10, color: S.gold, letterSpacing: 2, textTransform: "uppercase", fontFamily: S.body, fontWeight: 700, marginBottom: 12 }}>Support</div>
            {link("FAQ", "FAQ")}
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <p style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", fontFamily: S.body, margin: 0 }}>© 2026 CTS Empowerment & Training Solutions. All Rights Reserved.</p>
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <button onClick={function() { setPage("Privacy"); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", fontSize: 10, fontFamily: S.body, cursor: "pointer" }}>Privacy</button>
            <button onClick={function() { setPage("Terms"); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", fontSize: 10, fontFamily: S.body, cursor: "pointer" }}>Terms</button>
            <span style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: S.body }}>Last Updated: March 2026</span>
          </div>
        </div>
      </div>
    </footer>
