...
function generateHeartPDF(data, signature) {
  ...
  var css = '*{margin:0;padding:0;box-sizing:border-box}'
    ...
    + '.page{width:210mm;min-height:297mm;padding:10mm 12mm;position:relative;page-break-after:always}'
    ...
    + '@media print{.page{page-break-after:always;padding:8mm 10mm}.ov{display:none}}';
...
function createHeartFormFile(data, signature) {
  var today = new Date();
  var todayStr = String(today.getDate()).padStart(2,"0") + "/" + String(today.getMonth()+1).padStart(2,"0") + "/" + today.getFullYear();
  var v = function(val) { return (val || "").toString().toUpperCase() || "\u2014"; };
  var sigImg = signature ? '<img src="' + signature + '" style="height:48px;object-fit:contain" />' : '<div style="height:36px;border-bottom:1px solid #333;width:200px"></div>';
  var row = function(l,val) { return '<tr><td style="padding:4px 8px;background:#f0f4f8;font-weight:600;width:35%;border:1px solid #aaa;font-size:9px">' + l + '</td><td style="padding:4px 8px;border:1px solid #aaa;font-size:9px;text-transform:uppercase">' + v(val) + '</td></tr>'; };
  var html = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:9px;color:#111;max-width:210mm;margin:0 auto;padding:10mm}'
    + '.hdr{background:#0066B2;padding:8px 10px;color:#fff;margin-bottom:6px}.hdr h1{font-size:13px;margin:0}.hdr p{font-size:7px;margin:2px 0 0;opacity:.7}'
    + '.sec{background:#0066B2;color:#fff;padding:4px 8px;font-size:9px;font-weight:700;margin-top:8px}table{width:100%;border-collapse:collapse;margin-bottom:4px}'
    + '.ft{margin-top:8px;font-size:7px;color:#888;text-align:center;border-top:1px solid #ddd;padding-top:4px}'
    + '</style></head><body>'
    + '<div class="hdr"><h1>HEART/NSTA TRUST \u2014 APPLICATION FOR ADMISSION FORM</h1><p>August 2023 V2.0 | Auto-generated by CTS ETS on ' + todayStr + '</p></div>'
    + '<div class="sec">SECTION A \u2014 PERSONAL INFORMATION</div><table>'
    + row("Last Name", data.lastName) + row("First Name", data.firstName) + row("Middle Name", data.middleName) + row("Maiden Name", data.maidenName)
    + row("Gender", data.gender) + row("Date of Birth", data.dob) + row("Nationality", data.nationality)
    + row("TRN", data.trn) + row("NIS #", data.nis)
    + row("Email", data.email) + row("Phone 1", data.phone) + row("Phone 2", data.phone2)
    + row("Marital Status", data.maritalStatus) + row("Special Needs", data.specialNeeds === "Yes" ? "Yes \u2014 " + (data.specialNeedsType||"") : "No")
    + row("Country", data.country) + row("Parish", data.parish) + row("Street", data.address) + row("District", data.district) + row("Postal Zone", data.postalZone)
    + '</table>'
    + '<div class="sec">EMERGENCY CONTACTS</div><table>'
    + row("Contact #1 Name", data.emergencyName) + row("Contact #1 Phone", data.emergencyPhone) + row("Contact #1 Relationship", data.emergencyRelationship)
    + (data.emergency2Name ? row("Contact #2 Name", data.emergency2Name) + row("Contact #2 Phone", data.emergency2Phone) + row("Contact #2 Relationship", data.emergency2Relationship) : "")
    + '</table>'
    + '<div class="sec">SECTION B \u2014 PROGRAMME INFORMATION</div><table>'
    + row("Programme", data.programme) + row("Level", data.level) + row("Previously at HEART?", data.previousHeart || "No")
    + '</table>'
    + '<div class="sec">SECTION C \u2014 EDUCATION & EMPLOYMENT</div><table>'
    + row("Highest Qualification", data.highestQualification) + row("Last School", data.schoolLastAttended) + row("Year Completed", data.yearCompleted)
    + row("Employment Status", data.employmentStatus) + row("Employer", data.employer) + row("Job Title", data.jobTitle)
    + '</table>'
    + '<div class="sec">SECTION E \u2014 DECLARATION & SIGNATURE</div>'
    + '<div style="font-size:8px;color:#333;line-height:1.6;padding:6px 0">\u2611 All information is true and honestly presented. \u2611 I will notify of changes. \u2611 Application cannot be altered after submission. \u2611 Privacy consent given.</div>'
    + '<div style="margin:8px 0"><div style="font-size:7px;color:#888;margin-bottom:3px">SIGNATURE OF APPLICANT</div>' + sigImg + '</div>'
    + '<div style="font-size:9px;font-weight:600">Date: ' + todayStr + '</div>'
    + '<div class="ft">HEART/NSTA Trust \u2014 Application for Admission Form \u2014 August 2023 V2.0<br>Auto-generated and signed via CTS ETS (ctsetsjm.com)</div>'
    + '</body></html>';
  try {
    var blob = new Blob([html], { type: "text/html" });
    return new File([blob], "HEART_Application_" + (data.firstName||"").replace(/[^a-zA-Z]/g,"") + "_" + (data.lastName||"").replace(/[^a-zA-Z]/g,"") + ".html", { type: "text/html" });
  } catch(e) { return null; }
}
...
export default function HeartFormBuilder({ formData, onComplete }) {
  ...
  var _s2 = useState(false); var attached = _s2[0]; var setAttached = _s2[1];
  var data = {
  ...
  var handleConfirm = function() { var heartFile = createHeartFormFile(data, signature); setAttached(true); if (onComplete) onComplete(signature, heartFile); };
  var handleDownloadCopy = function() { generateHeartPDF(data, signature); };
  return (
    <div>
      <div style={{ padding: "16px 20px", borderRadius: 12, background: S.tealLight, border: "1px solid " + S.teal + "30", marginBottom: 24, display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: S.gray, fontFamily: S.body, lineHeight: 1.7, margin: 0 }}>
            This is the official HEART/NSTA Trust Application for Admission Form (August 2023 V2.0), auto-filled from your details. Review below, sign, then confirm. The signed form will be automatically attached to your application.
          </p>
        </div>
      </div>
      <div style={{ marginBottom: 24 }}><HeartFormPreview data={data} signature={signature} /></div>
      <div style={{ marginBottom: 24 }}><SignaturePad onSign={function(s) { setSignature(s); if (attached) { setAttached(false); } }} signatureData={signature} /></div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        {!attached ? (
          <button onClick={handleConfirm} disabled={!signature}
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "14px 28px", borderRadius: 8,
              background: signature ? S.emerald : "rgba(1,30,64,0.08)", color: signature ? "#fff" : S.grayLight,
              border: "none", fontSize: 14, fontWeight: 700, cursor: signature ? "pointer" : "not-allowed",
              fontFamily: S.body, boxShadow: signature ? "0 4px 16px rgba(46,125,50,0.2)" : "none" }}>
            {"\u2713"} Confirm & Attach HEART Form
          </button>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 20px", borderRadius: 8, background: S.emeraldLight, border: "1.5px solid " + S.emerald + "40" }}>
              <span style={{ width: 22, height: 22, borderRadius: "50%", background: S.emerald, display: "inline-flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12 }}>{"\u2713"}</span>
              <span style={{ fontSize: 13, color: S.emeraldDark, fontFamily: S.body, fontWeight: 700 }}>HEART form signed & attached to your application</span>
            </div>
            <button onClick={handleDownloadCopy}
              style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 20px", borderRadius: 8,
                background: "transparent", color: S.navy, border: "1.5px solid " + S.navy + "30",
                fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: S.body }}>
              {"\uD83D\uDCC4"} Download a Copy (PDF)
            </button>
          </div>
        )}
      </div>
      {!signature && !attached && <p style={{ fontSize: 11, color: S.gray, fontFamily: S.body, marginTop: 10 }}>Please sign above before confirming.</p>}
    </div>
  );
}
