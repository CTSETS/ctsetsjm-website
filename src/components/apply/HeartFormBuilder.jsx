...
function buildHeartHTML(data, signature) {
  var today = new Date();
  ...
  var css = '*{margin:0;padding:0;box-sizing:border-box}'
    ...
    + '.page{width:8.5in;padding:6mm 10mm;position:relative;page-break-after:always}'
    ...
    + '@page{size:8.5in 14in;margin:5mm}@media print{.page{page-break-after:always;padding:6mm 8mm}.ov{display:none}}';
  ...
  return { css: css, page1: page1, page2: page2, page3: page3 };
}

function createHeartFile(data, signature) {
  var b = buildHeartHTML(data, signature);
  var html = '<!DOCTYPE html><html><head><title>HEART Application</title>'
    + '<style>' + b.css + '</style></head><body>'
    + b.page1 + b.page2 + b.page3
    + '</body></html>';
  var blob = new Blob([html], { type: "text/html" });
  var fileName = "HEART_Application_" + (data.firstName || "").replace(/[^a-zA-Z]/g, "") + "_" + (data.lastName || "").replace(/[^a-zA-Z]/g, "") + ".html";
...
function generateHeartPDF(data, signature) {
  ...
  var html = '<!DOCTYPE html><html><head><title>HEART Application \u2014 ' + data.firstName + ' ' + data.lastName + '</title>'
    ...
    + '<div id="fc">' + b.page1 + b.page2 + b.page3 + '</div>'
    ...
    + 'html2pdf().set({margin:0,filename:fn,image:{type:"jpeg",quality:0.98},html2canvas:{scale:2,useCORS:true},jsPDF:{unit:"in",format:"legal",orientation:"portrait"},pagebreak:{mode:["css","legacy"]}})'
...
export default function HeartFormBuilder({ formData, onComplete }) {
  ...
  var _s2 = useState(false); var confirmed = _s2[0]; var setConfirmed = _s2[1];
  var data = {
  ...
  var handleConfirm = function() {
    var heartFile = createHeartFile(data, signature);
    setConfirmed(true);
    if (onComplete) onComplete(signature, heartFile);
  };
    ...
    <div>
      <div style={{ padding: "16px 20px", borderRadius: 12, background: S.tealLight, border: "1px solid " + S.teal + "30", marginBottom: 24, display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: S.gray, fontFamily: S.body, lineHeight: 1.7, margin: 0 }}>
            This is the official HEART/NSTA Trust Application for Admission Form (August 2023 V2.0), auto-filled from your details. Review below, sign, then click Confirm to automatically attach it to your application.
          </p>
        </div>
      </div>
      ...
      {!confirmed ? (
        <div>
          ...
          {!signature && <p style={{ fontSize: 11, color: S.gray, fontFamily: S.body, marginTop: 10 }}>Please sign above before confirming.</p>}
        </div>
      ) : (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", borderRadius: 10, background: S.emeraldLight, border: "1px solid " + S.emerald + "40", marginBottom: 12 }}>
            <span style={{ width: 24, height: 24, borderRadius: "50%", background: S.emerald, display: "inline-flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 700, flexShrink: 0 }}>{"\u2713"}</span>
            <span style={{ fontSize: 14, color: S.emerald, fontFamily: S.body, fontWeight: 700 }}>HEART form signed & attached to your application</span>
          </div>
          <button onClick={handleDownloadCopy}
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 8,
              background: "transparent", color: S.navy, border: "2px solid " + S.navy + "20",
