...
function ContactMethodCard({ icon, label, value, href, color, external }) {
  return (
    <a href={href} style={{ textDecoration: "none" }} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined}>
      <div style={{ background: "#fff", borderRadius: 22, padding: "30px 24px", border: `1px solid ${S.border}`, transition: "all 0.28s", boxShadow: "0 10px 28px rgba(15,23,42,0.04)", height: "100%" }}>
        <div style={{ width: 60, height: 60, borderRadius: 18, background: `${color}15`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, marginBottom: 18 }}>{icon}</div>
        <div style={{ fontSize: 11, color, letterSpacing: 1.8, textTransform: "uppercase", fontFamily: S.body, fontWeight: 800, marginBottom: 8 }}>{label}</div>
        <div style={{ fontSize: 18, color: S.navy, fontWeight: 800, fontFamily: S.heading, lineHeight: 1.35 }}>{value}</div>
      </div>
    </a>
  );
}
function AppointmentCard({ apt }) {
  return (
    <a href={apt.url} target="_blank" rel="noopener noreferrer" onClick={() => trackBookingClicked(apt.title)} style={{ textDecoration: "none" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "28px 24px", border: `1px solid ${S.border}`, transition: "all 0.28s ease", cursor: "pointer", boxShadow: "0 8px 24px rgba(15,23,42,0.04)", display: "flex", flexDirection: "column", height: "100%" }}>
...
function FAQItem({ faq, isOpen, onClick, delay = 0 }) {
  return (
    <Reveal delay={delay}>
      <div style={{ marginBottom: 12, borderRadius: 18, border: `1px solid ${isOpen ? S.coral : S.border}`, overflow: "hidden", background: "#fff", transition: "all 0.22s ease", boxShadow: isOpen ? `0 14px 30px ${S.coral}15` : "0 8px 20px rgba(15,23,42,0.03)" }}>
        <button onClick={onClick} style={{ width: "100%", padding: "20px 22px", background: isOpen ? S.coral : "#fff", border: "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, textAlign: "left", transition: "all 0.2s" }}>
...
export default function ContactPage({ setPage }) {
  ...
  const inp = { width: "100%", padding: "14px 18px", borderRadius: 12, border: `1px solid ${S.border}`, background: S.white, fontSize: 15, fontFamily: S.body, color: S.navy, outline: "none", boxSizing: "border-box", transition: "border-color 0.2s, box-shadow 0.2s" };
  const lbl = { fontSize: 12, color: S.navy, fontWeight: 700, fontFamily: S.body, display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 };
  const filteredFaqs = search.trim() ? FAQS.filter((f) => f.q.toLowerCase().includes(search.toLowerCase()) || f.a.toLowerCase().includes(search.toLowerCase())) : FAQS;

  const send = async () => {
    if (hp || Date.now() - start.current < 5000 || !captcha) return;
    if (!form.name.trim() || !form.email.trim() || !form.message.trim()) { alert("Please fill in your name, email, and a message."); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) { alert("Please enter a valid email address."); return; }
    setSending(true);
    try {
      const payload = { action: "submitcontact", form_type: "Contact Enquiry", contactName: form.name, email: form.email, phone: form.phone || "N/A", subject: form.subject || "General", message: form.message, timestamp: new Date().toISOString() };
  ...
  return (
    <PageWrapper bg={S.lightBg}>
      <div style={{ background: "linear-gradient(135deg, #0B1120 0%, #1E293B 58%, #0E8F8B 145%)", position: "relative", overflow: "hidden" }}>
        ...
        <Container style={{ position: "relative", paddingTop: 60, paddingBottom: 54 }}>
          <Reveal>
            ...
            <h1 style={{ fontFamily: S.heading, fontSize: "clamp(36px, 6vw, 66px)", lineHeight: 1.04, color: "#fff", fontWeight: 900, margin: "0 0 18px", maxWidth: 860 }}>Get help, ask questions, or book time with the team</h1>
            <p style={{ fontFamily: S.body, fontSize: "clamp(15px, 2vw, 19px)", lineHeight: 1.8, color: "rgba(255,255,255,0.82)", maxWidth: 780, margin: "0 0 24px" }}>This upgrade keeps the same form submission logic, booking links, CAPTCHA, FAQ search, and contact options — but makes the page feel more polished, trustworthy, and easier to use.</p>
      ...
      <Container style={{ paddingTop: 26 }}>
        <SectionHeader tag="Contact Options" title="Reach us in the way that fits you best" desc="Some visitors want to call, some want to message, and some want a scheduled conversation. The layout below preserves each of those paths while making them easier to scan and trust." accentColor={S.sky} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 22, marginBottom: 58 }}>
          {contactCards.map(([icon, label, value, href, color, external]) => <Reveal key={label}><ContactMethodCard icon={icon} label={label} value={value} href={href} color={color} external={external} /></Reveal>)}
        </div>
        <SectionHeader tag="Consultations" title="Book a free conversation with the team" desc="This section keeps the same booking routes, but presents them with clearer visual hierarchy so visitors can quickly choose the right appointment type." accentColor={S.violet} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 20, marginBottom: 70 }}>
          {appointments.map((apt) => <Reveal key={apt.title}><AppointmentCard apt={apt} /></Reveal>)}
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 28, alignItems: "start", marginBottom: 76 }}>
          <Reveal>
            <div>
              <SectionHeader tag="Message Us" title="Send a direct enquiry" desc="The form logic stays the same. This update improves the structure around it so the page feels calmer and more professional." accentColor={S.coral} />
              <div style={{ background: "#fff", borderRadius: 22, padding: 24, border: `1px solid ${S.border}`, boxShadow: "0 12px 30px rgba(15,23,42,0.04)" }}>
                <div style={{ display: "grid", gap: 16 }}>
                  ...
                  ].map(([label, text, color]) => <div key={label} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><div style={{ width: 12, height: 12, borderRadius: 999, background: color, marginTop: 5, flexShrink: 0 }} /><div><div style={{ fontSize: 12, color: S.navy, fontWeight: 800, fontFamily: S.body, letterSpacing: 1.1, textTransform: "uppercase", marginBottom: 4 }}>{label}</div><div style={{ fontSize: 14, color: S.gray, fontFamily: S.body, lineHeight: 1.7 }}>{text}</div></div></div>)}
                </div>
              </div>
            </div>
          </Reveal>
            ...
            {sent ? (
              ...
              <div style={{ background: S.white, borderRadius: 24, padding: "clamp(28px, 4vw, 40px)", border: `1px solid ${S.border}`, boxShadow: "0 12px 30px rgba(15,23,42,0.04)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20, marginBottom: 22 }}>
                  ...
                  <div><label style={lbl}>Subject</label><select style={{ ...inp, cursor: "pointer" }} value={form.subject} onChange={(e) => u("subject", e.target.value)}><option value="">Select topic</option>{["General Enquiry", "Programme Info", "Fees & Payment", "Application Help", "Employer / Group", "Technical Issue", "Feedback"].map((s) => <option key={s}>{s}</option>)}</select></div>
                </div>
                <div style={{ marginBottom: 24 }}><label style={lbl}>Your Message *</label><textarea style={{ ...inp, minHeight: 160, resize: "vertical", paddingTop: 16 }} value={form.message} onChange={(e) => u("message", e.target.value)} placeholder="How can we help you today?" /></div>
        ...
        <SectionHeader tag="Knowledge Base" title="Search frequently asked questions" desc="The FAQ search and accordion stay in place, but the section feels more intentional and easier to navigate." accentColor={S.teal} />
