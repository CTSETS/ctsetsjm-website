...
import { CAREER_OUTCOMES, PROGRAMMES } from "../constants/programmes";
...
const CAREER_PATHS = [
  { programme: "Customer Service", levels: "L2 → L3 Supervision", roles: ["Customer Service Rep", "Team Leader", "Supervisor", "Call Centre Manager"], sectors: "BPO, Retail, Hospitality, Banking", color: S.teal },
  { programme: "Business Administration", levels: "L3 → L4 → L5", roles: ["Office Administrator", "Operations Coordinator", "Office Manager", "General Manager"], sectors: "Corporate, Government, NGOs, SMEs", color: S.violet },
  { programme: "Human Resource Management", levels: "L4 → L5", roles: ["HR Officer", "HR Coordinator", "HR Manager", "HR Director"], sectors: "All sectors — every organisation needs HR", color: S.coral },
  { programme: "Industrial Security", levels: "L2 → L3", roles: ["Security Officer", "Shift Supervisor", "Operations Manager", "Security Director"], sectors: "Private Security, Hospitality, Corporate, Events", color: S.amber },
  { programme: "Entrepreneurship", levels: "L2 → L3", roles: ["Small Business Owner", "Freelance Consultant", "Social Enterprise Founder"], sectors: "Self-employment, Start-ups, Community Development", color: S.emerald },
];

export default function CareerOutcomesPage({ setPage }) {
  return (
    <PageWrapper bg="#F5F3EE">
      <SectionHeader tag="Where Can This Take You?" title="Career Paths at Every Level" desc="Every qualification opens real doors. Here's where CTS ETS graduates are heading." accentColor={S.emerald} />
      <Container>
        {/* Salary bands */}
        <Reveal>
          <div style={{ marginBottom: 48 }}>
            ...
            <p style={{ textAlign: "center", fontSize: 11, color: S.gray, fontFamily: S.body, marginTop: 14, fontStyle: "italic" }}>Salary ranges based on Jamaican market data. Actual salaries vary by employer, location, and experience.</p>
          </div>
        </Reveal>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          {CAREER_PATHS.map((cp, i) => (
            <Reveal key={cp.programme} delay={i * 0.06}>
              <div style={{ background: "#fff", borderRadius: 14, padding: "24px", border: "1px solid " + S.border, marginBottom: 16, display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                    {cp.roles.map((r, ri) => (
                      <span key={r}>
                        ...
                        {ri < cp.roles.length - 1 && <span style={{ color: cp.color, margin: "0 4px", fontSize: 10 }}>→</span>}
                      </span>
                    ))}
                  </div>
        ...
        <Reveal>
          <div style={{ textAlign: "center", marginTop: 24 }}>
            <Btn primary onClick={() => setPage("Apply")} style={{ background: S.coral, color: "#fff", fontSize: 15, padding: "16px 40px" }}>Start Your Career Path — Apply Now</Btn>
            <div style={{ marginTop: 12 }}><Btn onClick={() => setPage("Programmes")} style={{ fontSize: 13, border: "2px solid " + S.teal, color: S.teal }}>Browse All 25 Programmes</Btn></div>
          </div>
        </Reveal>
