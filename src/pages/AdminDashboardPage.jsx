...
export default function AdminDashboardPage() {
  ...
  const handleOtpChange = async (val) => {
    ...
    if (cleanVal.length === 6 && !loading) {
      setLoading(true);
      try {
        const data = await (await fetch(`${VERCEL_URL}?action=verifyotp&identifier=ADMIN&code=${cleanVal}&purpose=admin_login`)).json();
          ...
          <div style={{ background: C.card, borderRadius: 28, padding: "56px 42px", maxWidth: 500, width: "100%", textAlign: "center", border: `1px solid ${C.teal}40`, boxShadow: "0 24px 60px rgba(0,0,0,0.25)" }}>
            ...
            {loginStep === 1 && (
              <div style={{ animation: "fadeIn 0.3s" }}>
                ...
                <div style={{ textAlign: "center", fontSize: 13, color: C.gray, fontWeight: 700, marginTop: 12, marginBottom: 24, fontFamily: C.body }}>
                  Code expires in <span style={{ color: C.coral }}>{formatTime(timeLeft)}</span>
                </div>
