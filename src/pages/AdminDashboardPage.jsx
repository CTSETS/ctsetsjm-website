...
function AdminDashboardPage() {
  ...
  // ═══ LOGIN UI ═══
  if (!loggedIn) {
    const animStyles = `@keyframes pulseFortKnox { 0% { box-shadow: 0 0 0 0 rgba(14, 143, 139, 0.4); } 70% { box-shadow: 0 0 0 40px rgba(14, 143, 139, 0); } 100% { box-shadow: 0 0 0 0 rgba(14, 143, 139, 0); } } @keyframes floatShield { 0%, 100% { transform: translateY(0) scale(1); } 50% { transform: translateY(-15px) scale(1.05); } } @keyframes blinkLight { 0%, 100% { opacity: 1; transform: scale(1); box-shadow: 0 0 12px #2E7D32; } 50% { opacity: 0.3; transform: scale(0.8); box-shadow: 0 0 2px #2E7D32; } }`;
    return (
              ...
              <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>{processData(dashboard?.pendingPayments).map((p, i) => (
                  <tr key={i} style={{ background: i % 2 ? "#F8FAFC" : "#fff" }}>
                    ...
                    <TD>
                      {p?.receipt ? (
                        <a href={p.receipt} target="_blank" rel="noopener noreferrer" style={{ color: C.blue, fontWeight: 800, textDecoration: "underline" }}>
                          📄 View Receipt
                        </a>
                      ) : (
                        <span style={{ color: C.grayLight, fontSize: 12 }}>No Slip</span>
                      )}
                    </TD>
