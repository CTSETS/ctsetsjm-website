...
const WIPAY_CONFIG = {
  ...
  apiKey: "", // PASTE YOUR API KEY HERE — from WiPay Dashboard → Developer  
...
const NAV_GROUPS = [
  ...
  { label: "Programmes", children: [
    { label: "All Programmes", page: "Programmes" },
    { label: "Career Outcomes", page: "Careers" },
...
function NavDropdown({ group, page, setPage }) {
  ...
  if (!group.children) {
    return <button onClick={() => setPage(group.page)} style={{ padding: "8px 14px", borderRadius: 6, border: "none", background: isActive ? S.gold : "transparent", color: isActive ? S.navy : "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: isActive ? 700 : 500, cursor: "pointer", fontFamily: S.body, transition: "all 0.2s", whiteSpace: "nowrap" }}>{group.label}</button>;
  }
  return (
    <div style={{ position: "relative" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <button style={{ padding: "8px 14px", borderRadius: 6, border: "none", background: isActive ? S.gold : "transparent", color: isActive ? S.navy : "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: isActive ? 700 : 500, cursor: "pointer", fontFamily: S.body, transition: "all 0.2s", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4 }}>
        {group.label}
        <span style={{ fontSize: 8, opacity: 0.6, transition: "transform 0.2s", transform: hovered ? "rotate(180deg)" : "rotate(0deg)" }}>{"\u25BC"}</span>
      </button>
      {hovered && (
        <div style={{ position: "absolute", top: "100%", left: 0, background: S.navy, borderRadius: "0 0 10px 10px", boxShadow: "0 12px 32px rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.08)", minWidth: 200, paddingTop: 4, paddingBottom: 4, zIndex: 1001 }}>
          {group.children.map(child => (
            <button key={child.page} onClick={() => { setPage(child.page); setHovered(false); }} style={{ display: "block", width: "100%", padding: "10px 20px", border: "none", background: page === child.page ? "rgba(196,145,18,0.12)" : "transparent", color: page === child.page ? S.gold : "rgba(255,255,255,0.75)", fontSize: 13, fontWeight: page === child.page ? 700 : 400, cursor: "pointer", fontFamily: S.body, textAlign: "left", transition: "all 0.15s", borderLeft: page === child.page ? "3px solid " + S.gold : "3px solid transparent" }}
              onMouseEnter={e => { if (page !== child.page) e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
              onMouseLeave={e => { if (page !== child.page) e.currentTarget.style.background = "transparent"; }}
            >{child.label}</button>
          ))}
        </div>
      )}
        ...
        <div style={{ background: S.navy, borderTop: "1px solid rgba(255,255,255,0.08)", padding: "8px 0 16px" }} role="menu">
          {NAV_GROUPS.map(g => {
            if (!g.children) return (
              <button key={g.label} onClick={() => { setPage(g.page); setOpen(false); }} style={{ display: "block", width: "100%", padding: "12px 24px", border: "none", background: page === g.page ? "rgba(196,164,75,0.12)" : "transparent", color: page === g.page ? S.gold : "rgba(255,255,255,0.85)", fontSize: 14, fontWeight: page === g.page ? 700 : 400, cursor: "pointer", fontFamily: S.body, textAlign: "left", borderLeft: page === g.page ? "3px solid " + S.gold : "3px solid transparent" }}>{g.label}</button>
            );
            return (
              <div key={g.label}>
                ...
                {g.children.map(c => (
                  <button key={c.page} onClick={() => { setPage(c.page); setOpen(false); }} style={{ display: "block", width: "100%", padding: "10px 24px 10px 40px", border: "none", background: page === c.page ? "rgba(196,164,75,0.12)" : "transparent", color: page === c.page ? S.gold : "rgba(255,255,255,0.7)", fontSize: 13, fontWeight: page === c.page ? 700 : 400, cursor: "pointer", fontFamily: S.body, textAlign: "left", borderLeft: page === c.page ? "3px solid " + S.gold : "3px solid transparent" }}>{c.label}</button>
                ))}
              </div>
            );
          })}
...
function AboutPage() {
  ...
  const quoteStyle = { fontFamily: S.heading, fontSize: "clamp(17px,2.5vw,22px)", color: S.navy, lineHeight: 1.7, fontStyle: "italic", margin: 0 };
  const pStyle = { fontFamily: S.body, fontSize: 16, lineHeight: 1.9, color: "#2D3748", marginBottom: 20 };
  const emphStyle = { color: S.navy, fontWeight: 700 };
  const goldEmph = { color: S.gold, fontWeight: 700 };
    ...
    <PageWrapper bg={S.lightBg}>
      ...
      <Container>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }} className="resp-grid-2">
          <div>
            <p style={pStyle}>In Jamaica, thousands of talented individuals are held back — not by lack of ability, but by limited access to affordable, flexible, and professionally recognised training. CTS ETS was established to change that reality.</p>
            ...
            <p style={{ ...pStyle, marginBottom: 0 }}>From our base at 6 Newark Avenue in Kingston, we serve learners across all 14 parishes through flexible online delivery — because geography should never be a barrier to professional growth.</p>
          </div>
          <div style={{ background: S.navy, borderRadius: 16, padding: "clamp(28px,3vw,48px)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 2 }}>
              ...
              {[["Excellence", "We hold ourselves to the highest standards in everything we do"], ["Integrity", "Honest, transparent, and accountable — always"], ["Empowerment", "We equip people with the skills to transform their lives"], ["Service", "Called to serve — it's not just our name, it's our covenant"]].map(([v, d], i) => (
                <div key={v} style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: i < 3 ? 18 : 0 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(196,145,18,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: S.gold }} />
                  </div>
                  <div>
                    <span style={{ fontFamily: S.body, fontSize: 16, color: "#fff", fontWeight: 700, display: "block" }}>{v}</span>
                    <span style={{ fontFamily: S.body, fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.5 }}>{d}</span>
                  </div>
                </div>
              ))}
            </div>
...
export default function CTSApp() {
  ...
  if (loading) return <div style={{ transition: "opacity 0.6s ease", opacity: splashFading ? 0 : 1 }}><LoadingScreen onEnter={handleEnter} /></div>;
